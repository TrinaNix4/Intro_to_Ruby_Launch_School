...
in more recent versions of ruby, hashes maintain the orders in whch they're stored. the code will output the same results

# Regex

- stands for 'regular expression'
- a sequence of characters that form pattern matching rules, and is then applied to a string to look for matches

have many uses, for example:

- check if pattern 'ss' appears in 'mississippi'
- print out 3rd word of each sentence from a list of sentences
- find and replace all instances of Mrs with Ms
- does a string start with 'st'?
- does a string end with 'art'?
  and more

- creating regex's starts with a forward slash character (/)

- inside 2 forward slashes you can place any characters you would like to match with the string

- use the =~ operator to see if we have a match for our regex.

e.g. looking for 'b' in a string 'powerball'
...
- the order of the string and regex usually don't matter

- returned a 5 which means the first match that took place was at the fifth index of the string
